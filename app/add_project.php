<?php
// add_project.php - Form to add a new project
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $status = isset($_POST['status']) ? intval($_POST['status']) : 0; // Default to active
    $createdBy = $_SESSION['user_id']; // Get from logged-in user session
    ...
    if (!empty($projectCode) && is_project_code_duplicate($conn, $projectCode)) {
        $errors[] = "Project Code '$projectCode' already exists. Please choose a different one.";
    }

    if (empty($errors)) {
        if (add_project($conn, $projectName, $projectCode, $status, $createdBy)) {
            $_SESSION['success_message'] = "Project '$projectName' added successfully!";
...
<html lang="en">
    <head>
        ...
        <title>Add Project - <?= PROJECT_NAME?></title>
        <!-- Bootstrap CSS -->
            ...
            <div class="card card-spot p-4">
                ...
                <form method="POST" action="add_project.php">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="projectName" name="project_name" value="<?= htmlspecialchars($_POST['project_name'] ?? '') ?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="projectCode" class="form-label">Project Code</label>
                        <input type="text" class="form-control" id="projectCode" name="project_code" value="<?= htmlspecialchars($_POST['project_code'] ?? '') ?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="status" class="form-label">Status</label>
                        <select class="form-select" id="status" name="status">
                            <option value="0" <?= (isset($_POST['status']) && $_POST['status'] == 0) ? 'selected' : '' ?>>Active</option>
                            <option value="1" <?= (isset($_POST['status']) && $_POST['status'] == 1) ? 'selected' : '' ?>>Inactive</option>
                        </select>
                    </div>
